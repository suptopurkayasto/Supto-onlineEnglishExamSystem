...
@section('content')
    @if($grammarQuestions->count() > 0)
        ...
        <div class="card">
            <div class="card-header ">
                <h3 class="card-title float-left">All Grammar Questions Exam Student Marks</h3>
                <a href="{{ route('teachers.grammar-questions.create') }}" class="btn bg-gradient-indigo float-right">Add
                    Grammar Questions</a>
            </div>
            <!-- /.card-header -->
            <div class="card-body">
                <table id="example"
                    ...
                    <tbody>
                    @foreach($grammarQuestionExamStudentResults as $index => $grammarQuestionExamStudentResult)
                        <tr>
                            <td>{{ $index + 1 }}</td>
                            <td
                                class="text-bold">{{ $grammarQuestionExamStudentResults->got_marks }}</td>
                            <td class="text-center">
                                <a href="{{ route('teachers.grammar-questions.show', $grammarQuestion->id) }}"
                                   class="btn btn-primary btn-sm btn-block btn-hover-effect">View</a>
                            </td>
